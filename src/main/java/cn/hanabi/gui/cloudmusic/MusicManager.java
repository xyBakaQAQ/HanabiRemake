...
public class MusicManager {
    ...
    /**
     * Whether JavaFX (used for audio playback) is present on the runtime classpath.
     * Under Forge's LaunchClassLoader the JDK's bundled jfxrt.jar is not visible, so
     * the cloud-music feature is disabled gracefully instead of crashing the client.
     */
    public static final boolean JAVAFX_AVAILABLE = detectJavaFx();
    ...
    private static boolean detectJavaFx() {
        try {
            ...
            System.out.println("[Hanabi] JavaFX not available - cloud music disabled (" + t + ")");
            return false;
        }
    }
...
//		artistsScroll.setStepSize(.5f);
    ...
    public void play(Track track) {
        if (!JAVAFX_AVAILABLE) {
            return; // no audio backend; cloud music disabled
        }
        this.noUpdate = false;
